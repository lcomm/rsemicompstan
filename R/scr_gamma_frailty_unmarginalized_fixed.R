...
#' @export
scr_gamma_frailty_unmarginalized_fixed_stan <- function(x, z, yr, yt, dyr, dyt, use_priors = TRUE, 
  ...
  out <- rstan::sampling(stanmodels$scr_gamma_frailty_unmarginalized_fixed,
                         data = list(N = NROW(x), 
                                     z = z,
                                     P = NCOL(x),
                                     yr = yr,
